    ...
    try:
        try:
            ...
            if boto3_api_name in ['start_db_instance_automated_backups_replication', 'stop_db_instance_automated_backups_replication']:
                #if ('Invalid database identifier' in str(exception)) or ('InvalidParameterValue' in str(exception)):
                response = {'handled_exception_message': 'Source RDS db instance with ARN "{}" not found or operation is not valid for this instance.'.format(boto3_api_request_json['SourceDBInstanceArn'])}
            elif boto3_api_name in ['delete_db_cluster_automated_backup', 'delete_db_instance_automated_backup']:
                #if 'InvalidParameterValue' in str(exception):
                if boto3_api_name == 'delete_db_cluster_automated_backup':
                    response = {'handled_exception_message': 'RDS db cluster with resource id "{}" not found or operation is not valid for this cluster.'.format(boto3_api_request_json['DbClusterResourceId'])}
                else:
                    response = {'handled_exception_message': 'RDS db instance with resource id "{}" not found or operation is not valid for this instance.'.format(boto3_api_request_json['DbiResourceId'])}
            else:
